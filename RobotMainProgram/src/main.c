...
int main (void)
{
	...
	while(1)
	{
		...
		if (data == new_controller_loop)
		{
			...
			if(c%5==0)
			{
				Reza_state_feed_back();
				
				float nominal_v[4] ;
				float out_l[4];
				nominal_v[0]= fabs(u_saturated[0] / Robot.bat_v.full);
				nominal_v[1]= fabs(u_saturated[1] / Robot.bat_v.full);
				nominal_v[2]= fabs(u_saturated[2] / Robot.bat_v.full);
				nominal_v[3]= fabs(u_saturated[3] / Robot.bat_v.full);
